...
> Version 0.1.0 — A lightweight browser automation language with a clean, expressive syntax. Built on Playwright for modern web support.
...
`is` checks strict identity (same object):
...
| `Playwright errors`| Browser             | Timeout, element not found, etc.|
...
**"Element not found"** — the selector didn't match any element (Playwright error).
...
statement    = let | assign | go | fill | click | wait
             ...
             | continue | try | include | class | block
             | expression
...
export PLAYWRIGHT_CHROMIUM_FLAGS="--no-sandbox"
```
