...
**QR display**: The QR code prints as ASCII art directly to your terminal when a new login is required. You can also handle it programmatically:

```
conn = wa.connect()
conn.on_qr = function(data) {
    print "QR data: " + data.qr   // base64 string
    // You could render this as an image or send to a display
}
```
...
**Pairing code auth**: Instead of a QR code, you can authenticate by entering your phone number:
...
**Encrypted session export**: Save your session to an encrypted file for reuse:
...
The `wa_auth.z` script (included with Zen) automates this entire flow.
...
| `conn.newsletter_update_description(jid, desc)` | Update channel description |
...
x ??= "default"    # nullish assign (only if null)
x++                # increment
...
raise "error"          # alias for throw
assert cond            # debug assertion
assert cond, "message" # with custom message
...
let double = lambda x: x * 2        # lambda expression
let triple = (x) => x * 3           # arrow function
let add = (x, y) => x + y           # multi-param arrow
...
in not in              membership
and or not             logical
&& ||                  logical aliases (JS-style)
a if cond else b       ternary
x ?? y                 nullish coalescing (only null/undefined fallback)
& | ^                  bitwise AND, OR, XOR
~                      bitwise NOT
<< >>                  bit shift
x -> y                 range (inclusive)
...
typeof x               type name ("int", "string", "list", etc.)
